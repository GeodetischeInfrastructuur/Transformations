...
result = ','.join(['nr', 'lat_orig', 'lon_orig', 'h_orig', 'rdx_orig', 'rdy_orig', 'nap_orig', 'lat_calc', 'lon_calc', 'h_calc', 'rdx_calc', 'rdy_calc', 'nap_calc', 'lat_diff', 'lon_diff', 'h_diff', 'rdx_diff', 'rdy_diff', 'nap_diff']) + '\n'
...
for _, point in enumerate(Z001.readlines()):
    ...
    result += (f'{nr},{lat},{lon},{h},{rdx},{rdy},{nap},{lat_calc:.9f},{lon_calc:.9f},{h_calc:.4f},{rdx_calc:.4f},{rdy_calc:.4f},{nap_calc:.4f},{lat_diff:.9f},{lon_diff:.9f},{h_diff:.4f},{rdx_diff:.4f},{rdy_diff:.4f},{nap_diff:.4f}\n')
